...
- (void)ServiceRangeClick:(UIButton *)sender{
    ...
    NSString *string = [NSString stringWithFormat:@"seller.mappos?token=%@&userId=%d&%d",[GInfo shareClient].mGToken,[SUser currentUser].mUserId,(int)[[NSDate date] timeIntervalSince1970]];
    
    web.mUrl = [NSString stringWithFormat:@"%@",[APIClient APiWithUrl:string]];
    
    web.itblock = ^(NSString* addr,NSString* maps)
    {
        _address = addr;
        _maps = maps;
...
- (void)initView{
    ...
    mainView.frame = CGRectMake(0, 0, DEVICE_Width, 564);
...
- (void)assetsPickerController:(CTAssetsPickerController *)picker didFinishPickingAssets:(NSArray *)assets
{
    for (  ALAsset * one in assets  )
    {
        ...
        if (selectTag == 11) {
            bottomView.mCardZ.image = [UIImage imageWithCGImage: [[one defaultRepresentation] fullScreenImage]];
            [mAllImg replaceObjectAtIndex:0 withObject:bottomView.mCardZ.image];
        }else if (selectTag == 12){
        
             bottomView.mCardB.image = [UIImage imageWithCGImage: [[one defaultRepresentation] fullScreenImage]];
            [mAllImg replaceObjectAtIndex:1 withObject:bottomView.mCardB.image];

        }else{
        
            bottomView.mZenJian.image =  [UIImage imageWithCGImage: [[one defaultRepresentation] fullScreenImage]];
...
{
    if (selectTag == 11) {
        bottomView.mCardZ.image = image;
    }else if (selectTag == 12){
        
        bottomView.mCardZ.image = image;
    }else{
        bottomView.mZenJian.image =  image;
        
        uploadzj = image;
        
        mLImg = [[NSMutableArray alloc] initWithObjects:image,nil];
    }
}
...
//    
//    self.menuPopover = [[MLKMenuPopover alloc] initWithFrame:CGRectMake(mainView.mStoreTypeBtn.frame.origin.x, mainView.mStoreTypeBtn.frame.origin.y+mainView.mStoreTypeBtn.frame.size.height+70, mainView.mStoreTypeBtn.frame.size.width, 90) menuItems:self.menuItems];
//    
...
#pragma mark----营业执照
- (void)licenceAction:(UIButton *)sender{
    mType = 2;
    UIActionSheet *ac = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"拍照",@"从相册中选择", nil];
    ac.tag = 1002;
    [ac showInView:[self.view window]];
...
- (void)SaleTypeAction:(UIButton *)sender{
    ...
    viewController.itblock = ^(NSArray *arr){
        ...
        if (arr.count == 0) {
            ...
            if (mTTArr.count == 1) {
                [mainView.mSaleTypeBtn setTitle:[NSString stringWithFormat:@"%@",mTTArr[0]] forState:0];

            }
            if (mTTArr.count == 2) {
                [mainView.mSaleTypeBtn setTitle:[NSString stringWithFormat:@"%@,%@",mTTArr[0],mTTArr[1]] forState:0];

            }
            if (mTTArr.count >= 3){
                [mainView.mSaleTypeBtn setTitle:[NSString stringWithFormat:@"%@,%@,%@",mTTArr[0],mTTArr[1],mTTArr[2]] forState:0];
...
- (void)submitAction:(UIButton *)sender{
    ...
    if (![Util isMobileNumber:mainView.mPhoneNum.text]) {
...
- (CGRect)imageCropViewControllerCustomMaskRect:(RSKImageCropViewController *)controller
{
    if (mType == 1) {
        ...
        return   CGRectMake(self.view.center.x-mlicenceImg.frame.size.width, self.view.center.y-mlicenceImg.frame.size.height, mlicenceImg.frame.size.width, mlicenceImg.frame.size.height);
...
- (UIBezierPath *)imageCropViewControllerCustomMaskPath:(RSKImageCropViewController *)controller
{
    if (mType == 1) {
         ...
         return [UIBezierPath bezierPathWithRect:CGRectMake(self.view.center.x-mlicenceImg.frame.size.width, self.view.center.y-mlicenceImg.frame.size.height, mlicenceImg.frame.size.width, mlicenceImg.frame.size.height)];
