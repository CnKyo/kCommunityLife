...
- (IBAction)GetCodeClick:(id)sender {
    ...
    [SUser sendSM:_mPhoneTF.text block:^(SResBase *resb) {
        if (resb.msuccess) {
            [self timeCount];
...
- (IBAction)LoginClick:(id)sender {
    ...
    if( !_mIsUpdate )
    {
     ...
     {
         [SVProgressHUD showWithStatus:@"正在重置密码..." maskType:SVProgressHUDMaskTypeClear];
         [SUser reSetPswWithPhone:_mPhoneTF.text newpsw:_mNewPwdTF.text smcode:_mCodeTF.text  block:^(SResBase *resb, SUser *user) {
             if (resb.msuccess) {
                 [SVProgressHUD dismiss];
