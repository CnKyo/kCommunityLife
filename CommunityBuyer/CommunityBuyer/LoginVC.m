...
-(void)loadTopView
{
    ...
    for (int i =0; i<2; i++) {
        ...
        [btn setBackgroundColor:[UIColor colorWithRed:0.871 green:0.871 blue:0.871 alpha:1]];
...
-(void)topbtnTouched:(UIButton *)sender
{
    ...
    else
    {
        ...
        [sender setTitleColor:M_TCO forState:UIControlStateNormal];
        [tempBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        
        [tempBtn setBackgroundColor:[UIColor clearColor]];
        [sender setBackgroundColor:[UIColor colorWithRed:0.871 green:0.871 blue:0.871 alpha:1]];
...
- (IBAction)getCodeClick:(id)sender {
    ...
    [SUser sendSM:mQuikLogin.mQuikPhone.text block:^(SResBase *resb) {
        if (resb.msuccess) {
            [self timeCount];
...
- (IBAction)loginClick:(id)sender {
    ...
    if (_isPwd) {
        ...
        if (_codeTF.text == nil || [_codeTF.text isEqualToString:@""]) {
            [self showErrorStatus:@"验证码不能为空"];
            [_codeTF becomeFirstResponder];
            return;
        }
        ...
        if (_mNewPwd.text.length<=0) {
            ...
            [_mNewPwd becomeFirstResponder];
        }
        
    
        [SVProgressHUD showWithStatus:@"正在登录..." maskType:SVProgressHUDMaskTypeClear];
        [SUser reSetPswWithPhone:_phoneTF.text newpsw:_mNewPwd.text smcode:_codeTF.text block:^(SResBase *resb, SUser *user) {
            if (resb.msuccess) {
                [SVProgressHUD dismiss];
